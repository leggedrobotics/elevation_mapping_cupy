...
TEST(testSignedDistance3d, flatTerrain) {
  ...
  const auto signedDistanceAbove = signed_distance_field::signedDistanceAtHeight(map, testHeightAboveTerrain, resolution);
  ...
  const auto signedDistanceBelow = signed_distance_field::signedDistanceAtHeight(map, testHeightBelowTerrain, resolution);
...
TEST(testSignedDistance3d, randomTerrain) {
  ...
  for (float height = -1.0 - heightStep; height < 1.0 + heightStep; height+=heightStep) {
    ...
    const auto signedDistance = signed_distance_field::signedDistanceAtHeight(map, height, resolution);
