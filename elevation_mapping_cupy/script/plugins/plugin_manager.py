...
class PluginManger(object):
    ...
    def init(self, plugin_params: List[PluginParams], extra_params: List[Dict]):
        ...
        self.sync_plugins, self.sync_names, self.sync_indices = self.get_sync_plugins()
    ...
    def get_sync_plugins(self):
        ...
        names = []
        indices = []
        for i, obj in enumerate(self.plugins):
            if self.plugin_params[i].is_sync:
                plugins.append(obj)
                names.append(self.plugin_params[i].layer_name)
                indices.append(i)
        return plugins, names, indices

    def update_sync(self, elevation_map: cp.ndarray, layer_names: List[str]):
        for obj, idx in zip(self.sync_plugins, self.sync_indices):
            self.layers[idx] = obj(elevation_map, layer_names, self.layers, self.layer_names)
...
if __name__ == '__main__':
    plugins = [
                PluginParams(name="min_filter", layer_name="min_filter", is_sync=True),
                PluginParams(name="smooth_filter", layer_name="smooth", is_sync=True),
              ]
    extra_params = [
    ...
    # manager.init(plugins, extra_params)
    ...
    manager.update_sync(elevation_map, layer_names)
